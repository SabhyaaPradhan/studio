...
export default function DailySummaryPage() {
  ...
  // ✅ Fix: prevent hydration mismatch by setting date after mount
  useEffect(() => {
    setDate(new Date());
  }, []);

  useEffect(() => {
    if (isSubLoading || !date) {
      return;
    }
    ...
    if (user && isPro) {
      ...
      const unsubDaily = listenToAnalyticsDaily(
        user.uid,
        daysToFetch,
        (data) => {
          setDailyData(data);
      ...
      const unsubRealtime = listenToAnalyticsRealtime(
        user.uid,
        (data) => {
  ...
  }, [user, isPro, isSubLoading, date]);

  if (isSubLoading || (isLoading && isPro && !dailyData.length)) {
    return (
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-8">
        ...
        <p className="text-muted-foreground mb-4 max-w-md">
          Access daily insights and performance analytics by upgrading to a Pro plan.
        </p>
  ...
  const selectedDayData = date
    ? dailyData.find((d) => d.id === format(date, 'yyyy-MM-dd'))
    : null;
      ...
      <header className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            A quick overview of your activity, performance, and insights.
          </p>
        </div>
        <Popover>
          ...
          <PopoverContent className="w-auto p-0">
            <Calendar
              ...
              disabled={(d) =>
                d > new Date() ||
                d < new Date(new Date().setDate(new Date().getDate() - 30))
              }
            />
          </PopoverContent>
        </Popover>
      ...
      <main className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <section className="lg:col-span-3 grid gap-6 sm:grid-cols-2 md:grid-cols-4">
          {isLoading ? (
            ...
            <KpiCard
              title="Messages Sent"
              value={(selectedDayData?.total_messages ?? 0).toLocaleString()}
              trend="+0%"
            />
          )}
          {isLoading ? (
            ...
            <KpiCard
              title="Prompts Used"
              value={(selectedDayData?.assistant_messages ?? 0).toLocaleString()}
              trend="+0%"
            />
          )}
          {isLoading ? (
            ...
            <KpiCard
              title="Avg. Latency"
              value={`${(selectedDayData?.avg_latency_ms ?? 0).toFixed(0)}ms`}
              trend="-0%"
            />
          )}
          {isLoading ? (
            ...
            <KpiCard
              title="Total Tokens"
              value={(selectedDayData?.total_tokens ?? 0).toLocaleString()}
              trend="+0%"
            />
          )}
        </section>
        ...
        <section className="lg:col-span-2 space-y-6">
          {isLoading ? (
            ...
            <AiSummaryCard summaryData={dailyData ?? undefined} />
          )}
          {isLoading ? (
            <ChartsCardSkeleton />
          ) : (
            <ChartsCard dailyData={dailyData} />
          )}
        </section>
